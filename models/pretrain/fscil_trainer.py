...
class FSCILTrainer(Trainer):
    def __init__(self, args):
        ...
        # self.args.model_dir = self.args.save_pre_path + '/session0_max_acc.pth'
    ...
    def get_optimizer_base(self):
        ...
        optimizer = torch.optim.SGD(self.model.parameters(), self.args.lr_pretrain, momentum=0.9, nesterov=True,
    ...
    def train(self):
        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:
                ...
                for epoch in range(args.epochs_pretrain):
                    ...
                    # train pretrain sess
                    tl, ta = pre_train(self.model, trainloader, optimizer, scheduler, epoch, args,mask)
                    # test model with all seen class
                    tsl, tsa = test_pretrain(self.model, testloader, epoch, args, session)

                    # save better model
                    if (tsa * 100) >= self.trlog['max_acc'][session]:
                        self.trlog['max_acc'][session] = float('%.3f' % (tsa * 100))
                        self.trlog['max_acc_epoch'] = epoch
                        save_model_dir = os.path.join(args.save_pre_path, 'session' + str(session) + '_max_acc.pth')
                        torch.save(dict(params=self.model.state_dict()), save_model_dir)
                        torch.save(optimizer.state_dict(), os.path.join(args.save_pre_path, 'optimizer_best.pth'))
                        self.best_model_dict = deepcopy(self.model.state_dict())
                    ...
                    print('This epoch takes %d seconds' % (time.time() - start_time),
                          '\nstill need around %.2f mins to finish this session' % (
                                  (time.time() - start_time) * (args.epochs_pretrain - epoch) / 60))
                    scheduler.step()
                ...
                if not args.not_data_init:
                    ...
                    best_model_dir = os.path.join(args.save_pre_path, 'session' + str(session) + '_max_acc.pth')
                    print('Replace the fc with average embedding, and save it to :%s' % best_model_dir)
                    self.best_model_dict = deepcopy(self.model.state_dict())
                    torch.save(dict(params=self.model.state_dict()), best_model_dir)

                    self.model.module.mode = 'avg_cos'
                    tsl, tsa = test_pretrain(self.model, testloader, 0, args, session)
                    if (tsa * 100) >= self.trlog['max_acc'][session]:
                        self.trlog['max_acc'][session] = float('%.3f' % (tsa * 100))
        ...
        save_list_to_txt(os.path.join(args.save_pre_path, 'results.txt'), result_list)
    ...
    def set_save_path(self):
        ...
        self.args.save_pre_path = os.path.join('checkpoint', self.args.save_path)
        ensure_path(self.args.save_pre_path)

        return None

    def test_intergrate(self, model, testloader, epoch, args, session, validation=True):
        test_class = args.pretrain_class + session * args.way
        model = model.eval()
        vl = Averager()
        va = Averager()
        va5 = Averager()
        ...
        with torch.no_grad():
            for i, batch in enumerate(testloader, 1):
                ...
                emb = model.module.get_feature_test(data)
