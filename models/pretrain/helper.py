...
def pre_train(model, trainloader, optimizer, scheduler, epoch, args,mask):
    tl = Averager()
    ta = Averager()
    model = model.train()
    ...
    for i, batch in enumerate(tqdm_gen, 1):
        ...
        logits = model(data)
        logits_ = logits[:, :args.pretrain_class]
        ...
        if epoch>=args.loss_iter:
            ...
            pseudo_label1 = torch.argmax(mixed_logits[:, args.pretrain_class:], dim=-1) + args.pretrain_class  # new class label
...
def replace_base_fc(trainset, transform, model, args):
    ...
    with torch.no_grad():
        for i, batch in enumerate(trainloader):
            ...
            embedding = model.module.get_feature_test(data)
    ...
    for class_index in range(args.pretrain_class):
    ...
    model.module.fc.weight.data[:args.pretrain_class] = proto_list

    return model


def test_pretrain(model, testloader, epoch, args, session):
