...
def get_pretrain_dataloader(args):
    ...
    class_index = np.arange(args.pretrain_class)
    if args.dataset == 'cifar100':
    ...
    testloader = torch.utils.data.DataLoader(
        dataset=testset, batch_size=args.test_batch_size, shuffle=False, num_workers=32, pin_memory=True)
...
def get_meta_dataloader(args):
    txt_path = "data/index_list/" + args.dataset + "/session_" + str(0 + 1) + '.txt'
    class_index = np.arange(args.meta_class)
    if args.dataset == 'cifar100':
        trainset = args.Dataset.CIFAR100(root=args.dataroot, train=True, download=True,
                                         index=class_index, base_sess=True)
        testset = args.Dataset.CIFAR100(root=args.dataroot, train=False, download=False,
    ...
    sampler = CategoriesSampler(trainset.targets, args.meta_class//args.episode_way, args.episode_way,
                                args.episode_shot , args.episode_query,args)
...
def get_session_classes(args,session):
    class_list=np.arange(args.meta_class + session * args.way)
    return class_list